...
class UAnimatorStateHandler;

class UAnimator
{
private:
	UAnimationManager *mAnimationManager;
	UComPtr<IUIAnimationStoryboard> mStoryboard;
	UAnimatorStateHandler *mAnimatorStateListener;

	std::map<unsigned int, UComPtr<IUIAnimationVariable>> mVariableList;
...
public:
	...
	{
	public:
		virtual void onAnimationStart(UAnimator *animator) = 0;
		virtual void onAnimationEnd(UAnimator *animator) = 0;
		virtual void onAnimationCancel(UAnimator *animator) = 0;
	};

public:
	UAnimator(UAnimationManager *mgr);
	~UAnimator();
	...
	///</summary>
	void startTransition(unsigned int varIndex, std::shared_ptr<UTransition> transition);
