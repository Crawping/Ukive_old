...
UWindowSlave::UWindowSlave(UWindow *master)
{
	sUWSVtr = this;
	mMaster = master;
	mHandle = nullptr;
...
LRESULT CALLBACK UWindowSlave::messageHandler(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	{
	case WM_ACTIVATE:
		SetWindowPos(hWnd, mMaster->getWindowHandle(), 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE);
		return TRUE;
	...
	case WM_SIZE:
	{
		...
		::MoveWindow(mMaster->getWindowHandle(), newX, newY, newWidth, newHeight, FALSE);
		return 0;
	}
...
{
	if (mMaster->getWindowHandle() != nullptr 
		&& mMaster->isShowed() 
		&& mHandle == nullptr)
		initSlave();
}
