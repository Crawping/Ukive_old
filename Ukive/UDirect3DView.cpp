...
UDirect3DView::UDirect3DView(UWindow *wnd)
	:UWidget(wnd)
{
	initDirect3DView();
}

UDirect3DView::UDirect3DView(UWindow *wnd, int id)
	: UWidget(wnd, id)
{
	initDirect3DView();
}

UDirect3DView::~UDirect3DView()
{
	releaseState();
	releaseResourceView();
...
void UDirect3DView::initDirect3DView()
{
	setFocusable(true);
	setMinimumWidth(1);
	setMinimumHeight(1);
...
HRESULT UDirect3DView::createState()
{
	auto d3dDevice =
		mWindow->getApplication()->getDeviceManager()->getD3DDevice();
	auto d3dDeviceContext =
		mWindow->getApplication()->getDeviceManager()->getD3DDeviceContext();
...
void UDirect3DView::releaseState()
{
...
void UDirect3DView::setViewports(
	float x, float y, 
...
HRESULT UDirect3DView::createResourceView()
{
	auto d3dDevice =
		mWindow->getApplication()->getDeviceManager()->getD3DDevice();
	auto d3dDeviceContext =
	...
	UComPtr<ID3D11Texture2D> backBufferPtr;
...
void UDirect3DView::releaseResourceView()
{
...
void UDirect3DView::onSwapChainResized()
{
	HRESULT hr;
	...
	int widthSpec, int heightSpec)
{
	...
	case FIT:
	case EXACTLY:
		finalHeight = UMath::maximum(height, getMinimumHeight());
		break;
	case UNKNOWN:
		finalHeight = UMath::maximum(0, getMinimumHeight());
		break;
	}
...
void UDirect3DView::onDraw(UCanvas *canvas)
{
	UWidget::onDraw(canvas);
}

bool UDirect3DView::onInputEvent(UInputEvent *e)
{
...
void UDirect3DView::onSizeChanged(
	int width, int height, int oldWidth, int oldHeight)
{
	UWidget::onSizeChanged(width, height, oldWidth, oldHeight);
}


TerrainScene *UDirect3DView::getTerrainScene()
{
	return mScene;
}