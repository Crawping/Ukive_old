#include "UCommon.h"
#include "UWallpaperTransform.h"
#include "UWallpaperEffect.h"


UWallpaperEffect::UWallpaperEffect()
	:mRefCount(1)
{
	mTransform = nullptr;
}
...
IFACEMETHODIMP UWallpaperEffect::Initialize(
...
)
{
	...
	mTransform = new UWallpaperTransform();
...
IFACEMETHODIMP UWallpaperEffect::PrepareForRender(D2D1_CHANGE_TYPE changeType)
{
...
IFACEMETHODIMP UWallpaperEffect::SetGraph(_In_ ID2D1TransformGraph* pGraph)
{
	return E_NOTIMPL;
}

HRESULT __stdcall UWallpaperEffect::Register(_In_ ID2D1Factory1* pFactory)
{
...
HRESULT __stdcall UWallpaperEffect::CreateEffect(_Outptr_ IUnknown** ppEffectImpl)
{
	// 假设该效果初始引用计数为1。
	*ppEffectImpl = static_cast<ID2D1EffectImpl*>(new UWallpaperEffect());
...
HRESULT UWallpaperEffect::SetOffset(D2D_VECTOR_2F offset)
{
	return S_OK;
}

D2D_VECTOR_2F UWallpaperEffect::GetOffset() const
{
	return D2D1::Vector2F(0, 0);
}


IFACEMETHODIMP_(ULONG) UWallpaperEffect::AddRef()
{
	return InterlockedIncrement(&mRefCount);
}

IFACEMETHODIMP_(ULONG) UWallpaperEffect::Release()
{
...
IFACEMETHODIMP UWallpaperEffect::QueryInterface(_In_ REFIID riid, _Outptr_ void** ppOutput)
{
	if (ppOutput == nullptr)
		return E_INVALIDARG;
