...
UTextKeyListener::UTextKeyListener()
{
}
...
std::wstring UTextKeyListener::numpad(int keyCode)
{
	switch (keyCode)
	{
	case VK_NUMPAD0:
...
std::wstring UTextKeyListener::number(int keyCode)
{
	...
	{
	case 0x30:
		return isShiftPressed ? L")" : L"0";
	case 0x31:
		return isShiftPressed ? L"!" : L"1";
	case 0x32:
		return isShiftPressed ? L"@" : L"2";
	case 0x33:
		return isShiftPressed ? L"#" : L"3";
	case 0x34:
		return isShiftPressed ? L"$" : L"4";
	case 0x35:
		return isShiftPressed ? L"%" : L"5";
	case 0x36:
		return isShiftPressed ? L"^" : L"6";
	case 0x37:
		return isShiftPressed ? L"&" : L"7";
	case 0x38:
		return isShiftPressed ? L"*" : L"8";
	case 0x39:
		return isShiftPressed ? L"(" : L"9";
	}

	return L"";
}

std::wstring UTextKeyListener::alphabet(int keyCode, bool upper)
{
	if (upper)
	{
		switch (keyCode)
...
std::wstring UTextKeyListener::symbol(int keyCode)
{
	bool isShiftPressed = (::GetKeyState(VK_SHIFT) < 0);
...
bool UTextKeyListener::metaKey(int keyCode, bool down)
{
	switch (keyCode)
	{
	case VK_SHIFT:
...
void UTextKeyListener::backspace(UEditable *editable)
{
...
void UTextKeyListener::newline(UEditable *editable)
{
	if (editable->hasSelection())
		editable->replace(L"\n");
	else
...
bool UTextKeyListener::isNumpad(int keyCode)
{
	return (keyCode >= 0x60 && keyCode <= 0x6F);
}

bool UTextKeyListener::isNumber(int keyCode)
{
	return (keyCode >= 0x30 && keyCode <= 0x39);
}

bool UTextKeyListener::isAlphabet(int keyCode)
{
	return (keyCode >= 0x41 && keyCode <= 0x5A);
}

void UTextKeyListener::onKeyDown(
	UEditable *editable, bool canEdit, bool canSelect, int keyCode)
{
