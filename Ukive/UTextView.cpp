...
UTextView::UTextView(UWindow *wnd)
	:UWidget(wnd),
	mTextColor(D2D1::ColorF::Black),
	mSelectionBackgroundColor(D2D1::ColorF::SkyBlue)
{
	initTextView();
}

UTextView::UTextView(UWindow *wnd, int id)
	:UWidget(wnd, id),
	mTextColor(D2D1::ColorF::Black),
	mSelectionBackgroundColor(D2D1::ColorF::SkyBlue)
{
...
void UTextView::initTextView()
{
	...
	mTextBlink = new UTextBlink(this);
	mTextBlink->setColor(UColor::Blue500);

	mInputConnection = new UInputConnection(this);
	mTextKeyListener = new UTextKeyListener();

	mBaseText = new UEditable(L"");
...
int UTextView::computeVerticalScrollRange()
{
	int textHeight = (int)UMath::ceil(getTextHeight());
	int contentHeight = getHeight() - getPaddingTop() - getPaddingBottom();

	return UMath::maximum(0, textHeight - contentHeight);
}

int UTextView::computeHorizontalScrollRange()
{
	int textWidth = (int)UMath::ceil(getTextWidth());
	int contentWidth = getWidth() - getPaddingLeft() - getPaddingRight();

	return UMath::maximum(0, textWidth - contentWidth);
}


void UTextView::computeTextOffsetAtViewTop()
{
	BOOL isInside;
	BOOL isTrailingHit;
...
int UTextView::computeVerticalScrollOffsetFromTextOffset(uint32_t tOff)
{
	uint32_t actual;
...
int UTextView::determineVerticalScroll(int dy)
{
	...
	{
		if (dy < 0)
			finalY = UMath::maximum(0, getScrollY() + dy);
		else if (dy > 0)
			finalY = UMath::minimum(range, getScrollY() + dy);
	}

	return finalY - getScrollY();
}

int UTextView::determineHorizontalScroll(int dx)
{
	...
	{
		if (dx < 0)
			finalX = UMath::maximum(0, getScrollX() + dx);
		else if (dx > 0)
			finalX = UMath::minimum(range, getScrollX() + dx);
	}
...
void UTextView::scrollToFit(bool considerSelection)
{
	bool scrolled = false;
	...
	else
	{
		...
		int offsetY = UMath::minimum(maxScrollOffset, scrollOffset);
		if (offsetY != 0)
		{
			scrollBy(0, offsetY);
			scrolled = true;
...
void UTextView::blinkNavigator(int keyCode)
{
	if (keyCode == VK_LEFT)
	{
...
void UTextView::makeNewTextLayout(float maxWidth, float maxHeight, bool autoWrap)
{
	mTextLayout.reset();
...
void UTextView::locateTextBlink(int position)
{
	DWRITE_HIT_TEST_METRICS hitMetrics;
...
void UTextView::locateTextBlink(float textX, float textY)
{
...
void UTextView::onBeginProcess()
{
	++mProcessRef;
}

void UTextView::onEndProcess()
{
	--mProcessRef;
}


void UTextView::onAttachedToWindow()
{
	UWidget::onAttachedToWindow();
}

void UTextView::onDetachedFromWindow()
{
	UWidget::onDetachedFromWindow();
}

void UTextView::onFocusChanged(bool getFocus)
{
	UWidget::onFocusChanged(getFocus);
...
void UTextView::onWindowFocusChanged(bool windowFocus)
{
	UWidget::onWindowFocusChanged(windowFocus);
...
void UTextView::onScrollChanged(int scrollX, int scrollY, int oldScrollX, int oldScrollY)
{
	UWidget::onScrollChanged(scrollX, scrollY, oldScrollX, oldScrollY);

	computeTextOffsetAtViewTop();
}


void UTextView::onDraw(UCanvas *canvas)
{
	if (mIsSelectable)
	{
...
void UTextView::onMeasure(int width, int height, int widthSpec, int heightSpec)
{
	int finalWidth = 0;
	int finalHeight = 0;
	...
	case FIT:
	{
		...
		finalWidth = UMath::minimum(
			static_cast<int>(
				UMath::ceil(getTextWidth() + mTextBlink->getThickness()))
...
void UTextView::onSizeChanged(int width, int height, int oldWidth, int oldHeight)
{
...
bool UTextView::onInputEvent(UInputEvent *e)
{
	...
	case UInputEvent::EVENT_MOUSE_KEY_DOWN:
	{
		...
		if (e->getMouseKey() == UInputEvent::KEY_MOUSE_LEFT
			|| e->getMouseKey() == UInputEvent::KEY_MOUSE_RIGHT)
		{
			if (mTextActionMode != nullptr)
				mTextActionMode->close();
			...
			else if (e->getMouseKey() == UInputEvent::KEY_MOUSE_RIGHT)
			{
				mIsMouseRightKeyDown = isMouseKeyDown;
				//mIsMouseRightKeyDownOnText = isMouseKeyDownOnText;
			}

			if (e->getMouseKey() != UInputEvent::KEY_MOUSE_RIGHT || !hasSelection())
			{
...
bool UTextView::onCheckIsTextEditor()
{
	return mIsEditable;
}

UInputConnection *UTextView::onCreateInputConnection()
{
	return mInputConnection;
}


void UTextView::autoWrap(bool enable)
{
...
void UTextView::setIsEditable(bool editable)
{
...
void UTextView::setIsSelectable(bool selectable)
{
...
bool UTextView::isAutoWrap()
{
	return mIsAutoWrap;
}

bool UTextView::isEditable()
{
	return mIsEditable;
}

bool UTextView::isSelectable()
{
	return mIsSelectable;
}


void UTextView::setText(std::wstring text)
{
	mBaseText->replace(text, 0, mBaseText->length());
	mBaseText->setSelection(0);
}

void UTextView::setTextSize(float size)
{
	if (size == mTextSize) 
		return;
...
void UTextView::setTextColor(D2D1::ColorF color)
{
	mTextColor = color;
	invalidate();
}

void UTextView::setTextAlignment(DWRITE_TEXT_ALIGNMENT alignment)
{
...
void UTextView::setParagraphAlignment(DWRITE_PARAGRAPH_ALIGNMENT alignment)
{
...
void UTextView::setTextStyle(DWRITE_FONT_STYLE style)
{
	if (mTextStyle == style)
		return;
...
void UTextView::setTextWeight(DWRITE_FONT_WEIGHT weight)
{
...
void UTextView::setFontFamilyName(std::wstring font)
{
...
void UTextView::setLineSpacing(bool uniform, float spacingMultiple)
{
	if (uniform)
...
std::wstring UTextView::getText()
{
	return mBaseText->toString();
}

UEditable *UTextView::getEditable()
{
	return mBaseText;
}

float UTextView::getTextSize()
{
	return mTextSize;
}


void UTextView::setSelection(unsigned int position)
{
	mBaseText->setSelection(position);
}

void UTextView::setSelection(unsigned int start, unsigned int end)
{
	mBaseText->setSelection(start, end);
}

void UTextView::drawSelection(unsigned int start, unsigned int end)
{
	...
	for (UINT i = 0; i < hitTextMetricsCount; i++)
	{
		...
		block->rect.left = UMath::floor(hitTextMetrics[i].left);
		block->rect.top = UMath::floor(hitTextMetrics[i].top);
		block->rect.right = UMath::ceil(hitTextMetrics[i].left + hitTextMetrics[i].width + extraWidth);
		block->rect.bottom = UMath::ceil(hitTextMetrics[i].top + hitTextMetrics[i].height);
...
std::wstring UTextView::getSelection()
{
	return mBaseText->getSelection();
}

int UTextView::getSelectionStart()
{
	return mBaseText->getSelectionStart();
}

int UTextView::getSelectionEnd()
{
	return mBaseText->getSelectionEnd();
}

bool UTextView::hasSelection()
{
	return mBaseText->hasSelection();
}


uint32_t UTextView::getHitTextPosition(float textX, float textY)
{
	BOOL isInside;
	BOOL isTrailingHit;
	DWRITE_HIT_TEST_METRICS metrics;
...
bool UTextView::isHitText(float textX, float textY, uint32_t *hitPos)
{
	BOOL isInside;
	BOOL isTrailingHit;
	DWRITE_HIT_TEST_METRICS metrics;
...
URectF UTextView::getSelectionBound(unsigned int start, unsigned int end)
{
	if (end < start)
		return URectF();
...
void UTextView::computeVisibleRegion(URectF *visibleRegon)
{
...
void UTextView::onSelectionChanged(
...
void UTextView::onSpanChanged(
	USpan *span, UEditable::UEditWatcher::SpanChange action)
{
	DWRITE_TEXT_RANGE range;
	...
	{
	case USpan::USPAN_BASE_TYPE_TEXT:
	{
		switch (span->getType())
		{
		case USpan::TEXT_UNDERLINE:
	...
	case USpan::USPAN_BASE_TYPE_EFFECT:
		if (action == SpanChange::ADD)
		{
			UTextDrawingEffect *tdEffect = new UTextDrawingEffect();
	...
	case USpan::USPAN_BASE_TYPE_INTERACTABLE:
		break;

	case USpan::USPAN_BASE_TYPE_INLINEOBJECT:
		break;
	}
}


bool UTextView::canCut()
{
	return mIsEditable && mBaseText->hasSelection();
}

bool UTextView::canCopy()
{
	return mBaseText->hasSelection();
}

bool UTextView::canPaste()
{
	if (mIsEditable)
	{
...
void UTextView::performSelectAll()
{
...
bool UTextView::onCreateActionMode(TextActionMode *mode, UMenu *menu)
{
	if (this->canCopy())
...
bool UTextView::onPrepareActionMode(TextActionMode *mode, UMenu *menu)
{
	bool canCopy = this->canCopy();
	UMenuItem *copyItem = menu->findItem(MENU_ID_COPY);
	if (copyItem != nullptr)
	...
	UMenuItem *cutItem = menu->findItem(MENU_ID_CUT);
	if (cutItem != nullptr)
	...
	UMenuItem *pasteItem = menu->findItem(MENU_ID_PASTE);
	...
	UMenuItem *selectAllItem = menu->findItem(MENU_ID_SELECTALL);
...
bool UTextView::onActionItemClicked(TextActionMode *mode, UMenuItem *item)
{
...
void UTextView::onDestroyActionMode(TextActionMode *mode)
{
	mTextActionMode = nullptr;
}

void UTextView::onGetContentPosition(int *x, int *y)
{
	int left = 0;
	int top = 0;
	UWidget *parent = this;
