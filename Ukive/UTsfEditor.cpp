...
void UTsfEditor::setInputConnection(UInputConnection *ic)
{
	mInputConnection = ic;
}

void UTsfEditor::notifyTextChanged(bool correction, long start, long oldEnd, long newEnd)
{
...
void UTsfEditor::notifyStatusChanged(DWORD flags)
{
	if (mInputConnection && mAdviseSink.textStoreACPSink)
		mAdviseSink.textStoreACPSink->OnStatusChange(flags);
}

void UTsfEditor::notifyAttrsChanged(
	long start, long end, 
...
bool UTsfEditor::hasReadOnlyLock()
{
	return ((mCurLockType & TS_LF_READ) == TS_LF_READ);
}

bool UTsfEditor::hasReadWriteLock()
{
	return ((mCurLockType & TS_LF_READWRITE) == TS_LF_READWRITE);
}


STDMETHODIMP UTsfEditor::AdviseSink(REFIID riid, IUnknown *punk, DWORD dwMask)
{
	HRESULT hr;
	IUnknown *punkID;
...
STDMETHODIMP UTsfEditor::UnadviseSink(IUnknown *punk)
{
...
STDMETHODIMP UTsfEditor::RequestLock(DWORD dwLockFlags, HRESULT *phrSession)
{
	if (!mInputConnection)
		return E_FAIL;
...
STDMETHODIMP UTsfEditor::GetStatus(TS_STATUS *pdcs)
{
	if (pdcs == 0)
		return E_INVALIDARG;
...
STDMETHODIMP UTsfEditor::QueryInsert(
...
STDMETHODIMP UTsfEditor::GetSelection(
...
STDMETHODIMP UTsfEditor::SetSelection(ULONG ulCount, const TS_SELECTION_ACP *pSelection)
{
	std::wstringstream ss;
...
STDMETHODIMP UTsfEditor::GetText(
...
STDMETHODIMP UTsfEditor::SetText(
...
STDMETHODIMP UTsfEditor::GetFormattedText(LONG acpStart, LONG acpEnd, IDataObject **ppDataObject)
{
...
STDMETHODIMP UTsfEditor::GetEmbedded(LONG acpPos, REFGUID rguidService, REFIID riid, IUnknown **ppunk)
{
...
STDMETHODIMP UTsfEditor::QueryInsertEmbedded(const GUID *pguidService, const FORMATETC *pFormatEtc, BOOL *pfInsertable)
{
...
STDMETHODIMP UTsfEditor::InsertEmbedded(DWORD dwFlags, LONG acpStart, LONG acpEnd, IDataObject *pDataObject, TS_TEXTCHANGE *pChange)
{
...
STDMETHODIMP UTsfEditor::RequestSupportedAttrs(DWORD dwFlags, ULONG cFilterAttrs, const TS_ATTRID *paFilterAttrs)
{
	std::wstringstream ss;
...
STDMETHODIMP UTsfEditor::RequestAttrsAtPosition(LONG acpPos, ULONG cFilterAttrs, const TS_ATTRID *paFilterAttrs, DWORD dwFlags)
{
...
STDMETHODIMP UTsfEditor::RequestAttrsTransitioningAtPosition(LONG acpPos, ULONG cFilterAttrs, const TS_ATTRID *paFilterAttrs, DWORD dwFlags)
{
...
STDMETHODIMP UTsfEditor::FindNextAttrTransition(
...
STDMETHODIMP UTsfEditor::RetrieveRequestedAttrs(ULONG ulCount, TS_ATTRVAL *paAttrVals, ULONG *pcFetched)
{
...
STDMETHODIMP UTsfEditor::GetEndACP(LONG *pacp)
{
...
STDMETHODIMP UTsfEditor::GetACPFromPoint(TsViewCookie vcView, const POINT *pt, DWORD dwFlags, LONG *pacp)
{
...
STDMETHODIMP UTsfEditor::GetTextExt(TsViewCookie vcView, LONG acpStart, LONG acpEnd, RECT *prc, BOOL *pfClipped)
{
...
STDMETHODIMP UTsfEditor::GetScreenExt(TsViewCookie vcView, RECT *prc)
{
...
STDMETHODIMP UTsfEditor::GetWnd(TsViewCookie vcView, HWND *phwnd)
{
...
STDMETHODIMP UTsfEditor::InsertTextAtSelection(
...
STDMETHODIMP UTsfEditor::InsertEmbeddedAtSelection(DWORD dwFlags, IDataObject *pDataObject, LONG *pacpStart, LONG *pacpEnd, TS_TEXTCHANGE *pChange)
{
	return S_OK;
}


STDMETHODIMP UTsfEditor::OnStartComposition(
...
STDMETHODIMP UTsfEditor::OnUpdateComposition(
	__RPC__in_opt ITfCompositionView *pComposition,
...
STDMETHODIMP UTsfEditor::OnEndComposition(
	__RPC__in_opt ITfCompositionView *pComposition)
{
	return S_OK;
}
...
STDMETHODIMP UTsfEditor::QueryInterface(REFIID riid, void **ppvObj)
{
	if (ppvObj == 0)
		return E_INVALIDARG;
...
STDMETHODIMP_(ULONG) UTsfEditor::AddRef(void)
{
	return InterlockedIncrement(&mRefCount);
}

STDMETHODIMP_(ULONG) UTsfEditor::Release(void)
{
